...
const notify = {
    ...
    login: () => {
        ...
        //update users:
        document.querySelector("#users-list").innerHTML = users2Html([username]);
    }
}
...
const chat2Html = chat => {
    if (chat.username == username) {
        return `<p class="right"><strong>You:   </strong>${chat.text}</p> `
    } else {
        return `<p class="left"><strong>${chat.username}:   </strong>${chat.text}</p> `
    }
}
...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        document.querySelector("#users-list").innerHTML = users2Html(data.active_users);
        console.log(data);
        ...
        document.querySelector("#chat").innerHTML += chat2Html(data);
        console.log(data);
    } else {
